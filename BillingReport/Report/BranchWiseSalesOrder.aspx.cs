...
public partial class Report_BranchWiseSalesOrder : System.Web.UI.Page
{
    ...
    protected void ChartMonth_Click(object sender, ImageMapEventArgs e)
    {
        try
        {
            ...
            Session["MonthName"] = getMonthName;

            DisplayCountrySalesOrder_BasedOn_Month_And_Branch(Convert.ToString(Session["BranchName"]), Convert.ToString(Session["MonthName"]));
    ...
    protected void DrawMonthWiseDetails()
    {
        ...
        lblBranchName.Text = Session["BranchName"].ToString() + " " + ViewState["Fyear"].ToString();
        lblMsg.Text = "";
    ...
    void DisplayCountrySalesOrder_BasedOn_Month_And_Branch(string BranchName, string MonthName)
    {
        objDashBoard.Fromdate = ViewState["FromDate"].ToString();
        objDashBoard.Todate = ViewState["ToDate"].ToString();
        objDashBoard.BranchName = BranchName.Trim();
        objDashBoard.MonthID = GetMonthId(MonthName);

        DataSet dsDetails = new DataSet();
        dsDetails = objDashBoard.GetCountrySalesOrder_BasedOn_Month_branch();

        if (dsDetails.Tables[0].Rows.Count > 0)
        {
            lblMsg.Text = "";
            Session["CountrySalesOrder"] = dsDetails.Tables[0];
            DrawCountrySalesOrder();
    ...
    protected void DrawCountrySalesOrder()
    {
        try
        {
            ...
            dtGetCountryDetails = (DataTable)Session["CountrySalesOrder"];
            ...
            Chart_Country_SalesOrder.ChartAreas["ChartArea_Country_SalesOrder"].AxisX.IsMarginVisible = false;

            Chart_Country_SalesOrder.ChartAreas["ChartArea_Country_SalesOrder"].AxisX.Interval = .5;
            Chart_Country_SalesOrder.Series["Series_Country_SalesOrder"]["PointWidth"] = "0.4";
            Unit height = new Unit(1000);
            Chart_Country_SalesOrder.Height = height;

            title.Text = "Country Sales Order Branch : " + Session["BranchName"].ToString() + " & Month : " + Session["MonthName"].ToString();

            title.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(26)))), ((int)(((byte)(59)))), ((int)(((byte)(105)))));
            Chart_Country_SalesOrder.Titles[0] = title;

            if (rbtPercentage.Checked == true)
            {
                Chart_Country_SalesOrder.Series["Series_Country_SalesOrder"].Points.DataBindXY(dvAllCountryAmount, "countryname", dvAllCountryAmount, "totalorderper");
                Chart_Country_SalesOrder.Series["Series_Country_SalesOrder"].PostBackValue = "#AXISLABEL";
                Chart_Country_SalesOrder.Series["Series_Country_SalesOrder"].LabelFormat = "{0:n2}" + "%";
            }
            else
            {
                Chart_Country_SalesOrder.Series["Series_Country_SalesOrder"].Points.DataBindXY(dvAllCountryAmount, "countryname", dvAllCountryAmount, "totalorder");
                Chart_Country_SalesOrder.Series["Series_Country_SalesOrder"].PostBackValue = "#AXISLABEL";
            }
