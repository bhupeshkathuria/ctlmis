...
public partial class Report_CountryWiseSalesOrder : System.Web.UI.Page
{
    ...
    {
        string MonthName = e.PostBackValue;
        Session["MonthName"] = MonthName;

        DisplayBranchSalesOrder(Convert.ToString(Session["CountryName"]), MonthName);

        DisplayMonthWiseCountrytotalSalesOrder(Convert.ToString(Session["CountryName"]));
        DrawGraph();
        panelChart2.Visible = true;
        lblMsg.Text = "";
    ...
    protected void DrawMonthWise()
    {
        ...
        if (rbtnpersentagemonth.Checked == true)
        {
            ...
            title.Text = "Country " + Session["CountryName"] + " (Month Wise) Sales Order";
            title.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(26)))), ((int)(((byte)(59)))), ((int)(((byte)(105)))));
    ...
    void DisplayBranchSalesOrder(string CountryName, string MonthName)
    {
        try
        {
            objDashBoard.Fromdate = ViewState["FromDate"].ToString();
            ...
            dsCountryDetailsMonthWise = objDashBoard.GetBranchSalesOrderDetailsBasedOnCountryAndMonth();
            if (dsCountryDetailsMonthWise.Tables[0].Rows.Count > 0)
            {
                Session["BranchOrderDetails"] = dsCountryDetailsMonthWise.Tables[0];
                DrawBranchGraph();
            }
            else
            {
    ...
    void DrawBranchGraph()
    {
        ...
        dtGetBranchDetails = (DataTable)Session["BranchOrderDetails"];
        ...
        if (dvAllBranchAmount.Count > 0)
        {
            ...
            ChartCountry_Month_Branch_Orders.ChartAreas["ChartAreaBranch"].AxisX.IsMarginVisible = false;

            ChartCountry_Month_Branch_Orders.ChartAreas["ChartAreaBranch"].AxisX.Interval = .5;
            ChartCountry_Month_Branch_Orders.Series["SeriesBranch"]["PointWidth"] = "0.4";

            Unit height = new Unit(650);
            ChartCountry_Month_Branch_Orders.Height = height;
            title.Text = "Branch Sales Order  Country : " + Session["CountryName"].ToString() + " & Month : " + Session["MonthName"].ToString();
            ChartCountry_Month_Branch_Orders.Titles[0] = title;


            if (rbtPercentage.Checked == true)
            {
                ChartCountry_Month_Branch_Orders.Series["SeriesBranch"].Points.DataBindXY(dvAllBranchAmount, "branchname", dvAllBranchAmount, "totalorderper");
                ChartCountry_Month_Branch_Orders.Series["SeriesBranch"].PostBackValue = "#AXISLABEL";
                ChartCountry_Month_Branch_Orders.Series["SeriesBranch"].LabelFormat = "{0:n2}" + "%";
            }
            else
            {
                ChartCountry_Month_Branch_Orders.Series["SeriesBranch"].Points.DataBindXY(dvAllBranchAmount, "branchname", dvAllBranchAmount, "totalorder");
                ChartCountry_Month_Branch_Orders.Series["SeriesBranch"].PostBackValue = "#AXISLABEL";
            }
        }
    }
