...
public partial class MISReport_LowHighValueReport : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = new DataSet();
        ds = (DataSet)Session["InvoiceReport"];
        GVTrip.DataSource = (DataSet)Session["InvoiceReport"];

        GVTrip.PageSize = 20000;
        GVTrip.DataBind();

        string myFileName = "Invoice_High_Low" + "_" + DateTime.Now.Day + "_" + DateTime.Now.Month + "_" + DateTime.Now.Year + "_" + DateTime.Now.Hour + "_" + DateTime.Now.Minute + "_" + DateTime.Now.Second + ".xls";
    ...
    protected void BindData()
    {
        ...
        if (txtAmount.Text.ToString().Length > 0)
        {
            try
            {
                amountINR = Convert.ToDouble(txtAmount.Text);
            }
            catch (Exception ex)
            {
                lblmsg.Text = "Please input Numeric Value only";
                return;
            }
        }

        if (ddlHighLow.SelectedItem.Text == "Less Than")
        {
            strHighLowValue = "<=";
        }
        else
        ...
        lblmsg.Text = "";
        dsInvoice = objInvoice.GetHighLowValueInvoiceReport(year, month, countryid, strHighLowValue, amountINR);

        RadGrid1.DataSource = dsInvoice.Tables[0];
        RadGrid1.DataBind();
        Session["InvoiceReport"] = dsInvoice;
    ...
    protected void btnClearPaging_Click(object sender, EventArgs e)
    {
        if (btnClearPaging.Text == "Show Paging")
        {
            RadGrid1.AllowPaging = true;
            btnClearPaging.Text = "Show All Records";
            BindData();
        }
        else
        {
            RadGrid1.AllowPaging = false;
            btnClearPaging.Text = "Show Paging";
            BindData();
        }
