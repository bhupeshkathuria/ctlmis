...
public partial class Rebelfone_Rpt_Adcost : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //string from ="4/01/2014";// DateTime.Now.Month.ToString() + "/01/" + DateTime.Now..Year.ToString();
    ...
    {
        SqlConnection con = new SqlConnection(connection);
        SqlDataAdapter adp = new SqlDataAdapter("Select top 1 * from google_adcost order by id desc", connection);
        ...
        decimal adcost = Convert.ToDecimal(ds_getLastRowData.Tables[0].Rows[0]["adcost"].ToString());
        DateTime dtf = Convert.ToDateTime(_fromdate);
        DateTime dtt = dtf.AddMonths(1);
        dtt = dtt.AddDays(-1);
        string retdata=VisitsNumber(dtf, dtt, "ga:adCost");
        if (Convert.ToDecimal(retdata) > adcost)
        {
            SqlCommand cmd = new SqlCommand("UPDATE google_adcost set adcost='" + retdata + "' where id='" + Convert.ToInt32(ds_getLastRowData.Tables[0].Rows[0]["id"]) + "'");
    ...
    void InsertNewData(DateTime senddate)
    {
        ...
        if (senddate.Year == DateTime.Now.Year)
        {
            if (senddate.Month != DateTime.Now.Month)
            {
                ...
                DateTime dt_to = dt_from.AddMonths(1);
                dt_to = dt_to.AddDays(-1);
                string retdata = VisitsNumber(dt_from, dt_to, "ga:adCost");
                if (!string.IsNullOrEmpty(retdata))
                {
                }
                else
                ...
                SqlCommand cmd = new SqlCommand("Insert into google_adcost(monthid,yearname,adcost)values('" + Convert.ToInt32(dt_from.Month) + "','" + Convert.ToInt32(dt_from.Year) + "','" + Convert.ToDecimal(retdata) + "')");
                cmd.Connection = con;
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
    ...
    public DataTable Getdata(string matrics)
    {
        ...
        for (int year = 2009; year <= Convert.ToInt16(DateTime.Now.Year); year++)
        {
    ...
    private void DisplayReport()
    {
        ...
        try
        {
            ...
            ds = objVisitors.GetGoogleAdCost(Convert.ToInt32(drpFrom.SelectedValue), Convert.ToInt32(drpTo.SelectedValue));
            ...
            if (ds.Tables["yearwisesale"].Rows.Count > 0)
            {
                ...
                rptDoc.Load(Server.MapPath("~/Rebelfone_RPT/Rpt_MainAdCost.rpt"));
                ...
                foreach (ReportObject repOp in rptDoc.ReportDefinition.ReportObjects)
                {
                    ...
                    if (SubRepName == "rpt_Adcost.rpt")
                    {

                        subRepDoc.SetDataSource(ds.Tables["yearwisesale"]);

                    }
                    if (SubRepName == "Rpt_AdcostYear.rpt")
                    {
                        subRepDoc.SetDataSource(ds.Tables["AdcostYear"]);
                    }
    ...
    //{
    //    DataTable dt = Getdata("ga:adCost");
    //    gvAdvertising.DataSource = dt;
