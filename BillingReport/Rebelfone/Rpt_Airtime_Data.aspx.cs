...
public partial class Rebelfone_Rpt_Airtime_Phone : System.Web.UI.Page
{
    ...
    private void GetPhoneMainReport()
    {
        ...
        DataSet ds = objReport.GetAirtime(start_date, end_date);

        DataSet ds1 = objReport.GetAirtime(startdate2, end_date2);

        DataSet ds2 = objReport.GetPhone(start_date, end_date);

        DataSet ds22 = objReport.GetPhone(startdate2, end_date2);

        DataSet ds3 = objReport.GetDatabundle(start_date, end_date);

        DataSet ds4 = objReport.GetDatabundle(startdate2, end_date2);

        DataTable dt = ds.Tables[0];

        ReportDocument rptDoc = new ReportDocument();

        rptDoc.Load(Server.MapPath("~/Rebelfone_RPT/Rpt_Main_Airtime.rpt"));
        ...
        if (ds1.Tables[0].Rows.Count > 0)
        {
            ...
            
            {
                if (repOp.Kind == ReportObjectKind.SubreportObject)
                {
                    ...
                    if (SubRepName == "rptGetAirtime.rpt")
                    {
                        subRepDoc.SetDataSource(ds.Tables[0]);
                    } 
                    if (SubRepName == "rptGetAirtime2.rpt")
                    {
                        subRepDoc.SetDataSource(ds1.Tables[0]);
                    }
                    if(SubRepName=="rptGetAirtimeYearWise.rpt")
                    {
                        subRepDoc.SetDataSource(ds1.Tables[0]);
                    }
                    ...
                    if (SubRepName == "Rpt_Databundle.rpt")
                    {
                        subRepDoc.SetDataSource(ds3.Tables[0]);
                    }

                    if (SubRepName == "Rpt_Databundle2.rpt")
                    {
                        subRepDoc.SetDataSource(ds4.Tables[0]);
                    }

                    if (SubRepName == "Rpt_Databundle3.rpt")
                    {
                        subRepDoc.SetDataSource(ds4.Tables[0]);
                    }
                }
